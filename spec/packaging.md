...
`mise run icon` renders each size the `.icns` format wants from the 1024 result with CoreGraphics,
then calls `iconutil`. It runs from the artwork every time, and the rendered
set lives under `target/`. It renders nothing for the tray, whose frames the application draws
as they are needed from the glyph's geometry; see [ui.md](ui.md).
